pits_calculations <- function(df) {
  df |>
    mutate(
      ...
      snow_density_kilograms_meters_cubed = if_else(
        snow_depth_centimeters == 0 | is.na(snow_depth_centimeters),
        NA,
        snow_weight_kilograms / (pi * ((0.0465836 / 2)^2) * (snow_depth_centimeters * 0.01)),
      ),
